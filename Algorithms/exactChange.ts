type cidTuple = [string, number];

export function checkCashRegister(price: number, cash: number, cid: cidTuple[]): string | cidTuple[] {
    ...
    if (parseFloat(changeDue) > parseFloat(drawerTotal)) { return "Insufficient Funds"; }

    //Is exactly the right amount in the drawer?
    if (parseFloat(changeDue) === parseFloat(drawerTotal)) { return "Closed"; }
    ...
    validDenominations.forEach(a => {
        ...
        let val: number = denominations[a];
        let blown = false;
        let thisVal = 0;
