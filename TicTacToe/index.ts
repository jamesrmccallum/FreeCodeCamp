interface Window { game: TicTacToe }
...
class TicTacToe {
    ...
    private playerChar: number;
    private cpuChar: number;
    private gameActive: boolean;
    private _state: number[];

    constructor(char: number, grid?: string[]) {

        this._container = <HTMLDivElement>document.querySelector('#grid');
        this._statusBar = <HTMLDivElement>document.querySelector('#statusBar');
        this.buildGrid();
    ...
    private buildGrid() {
        ...
        for (let i = 0; i < 9; i++) {
            ...
            else rows[2].appendChild(divs[i])
        }
    ...
    private updateStatus(txt: string) {

        this._statusBar.innerText = txt;
    }

    private fillCell(i: number, char: number) {

        this._state[i] = char;
        this._grid[i].innerText = char === 1 ? 'x' : 'o';

        if (this.isWin(this.grid, false)) return this.gameEnd('You win');
        if (this.isWin(this.grid, true)) return this.gameEnd('CPU wins');
        if (!this.getEmptySquares(this.grid).length) return this.gameEnd('Draw');
    }

    private gameEnd(string: string) {
        this.gameActive = false;
        this.updateStatus(string);
    }

    /** Return the index of each empty square in the grid */
    private getEmptySquares(grid: number[]) {
    ...
    */
    click(ev: Event) {
        let square = (<HTMLDivElement>ev.target)
        ...
        if (this.gameActive) setTimeout(() => this.respond(), 50);
     ...
     * */
    private miniMax(grid: number[], cpuPlay: boolean, depth: number) {

        //Player Wins
        if (this.isWin(grid, false)) return depth - 10;

        //CPU Wins
        if (this.isWin(grid, true)) return 10 - depth;
    ...
    private isWin(grid: number[], cpuPlay: boolean) {
...
function main() {
    ...
    function makeChoice(e: Event) {
        let char = (<HTMLDivElement>e.target).innerText.toLowerCase();
